...
Mac mini: Tailscale Serve
    ...
    └─ 9443 → Open WebUI 127.0.0.1:3001
...
- 3001番と9443番を推奨。使用中の場合も既存アプリを終了させず、新規側へ近い空きポートを自動で一時割当する（明示指定も可）。
...
Mac miniのブラウザで`http://127.0.0.1:3001`を開き、**自分用の管理者1名だけ**を作成してください。導入時に自動割当された場合はその表示番号を使います。MacBookの`127.0.0.1`ではありません。初回はloopbackのHTTP、遠隔用のServe経路はまだ追加しません。
